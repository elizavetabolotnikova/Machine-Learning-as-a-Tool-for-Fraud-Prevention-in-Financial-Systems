class ModelTrainer:
    ...
    def __init__(self, X_train, y_train, X_val, y_val, X_test, y_test, feature_columns, cat_columns_for_catboost):
    ...
    def evaluate_model(self, name, y_true, y_prob, y_pred=None, threshold=0.5):
        ...
        metrics = {'ROC-AUC': roc_auc_score(y_true, y_prob), 'PR-AUC': average_precision_score(y_true, y_prob), 'F1': f1_score(y_true, y_pred), 'F2': fbeta_score(y_true, y_pred, beta=2), 'F0.5': fbeta_score(y_true, y_pred, beta=0.5), 'Precision': precision_score(y_true, y_pred, zero_division=0), 'Recall': recall_score(y_true, y_pred), 'Log Loss': log_loss(y_true, y_prob), 'Threshold': threshold}
    ...
    def train_catboost(self):
        print('\n--- CatBoost ---')
        cat_indices = [i for i, col in enumerate(self.feature_columns) if col in self.cat_columns_for_catboost]
...
trainer = ModelTrainer(X_train, y_train, X_val, y_val, X_test, y_test, feature_columns=preparer.feature_columns, cat_columns_for_catboost=preparer.cat_columns)
trainer.train_all()
