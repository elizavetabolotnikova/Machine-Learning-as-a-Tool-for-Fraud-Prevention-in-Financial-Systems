class RuleBasedFilter:
    ...
    def apply(self, X, return_details=False):
        ...
        self.rule_stats['__summary__'] = {'total': n, 'blocked_by_rules': int((rule_predictions == 1).sum()), 'safe_by_rules': int((rule_predictions == 0).sum()), 'grey_zone': int(mask_grey.sum())}
        if return_details:
...
if 'puid_orders_1h_without_refunds' in X_train.columns and 'order_loan' in X_train.columns:
    ...
    rule_filter.add_rule('extreme_velocity_1h', lambda X, tv=orders_1h_p95: (X['puid_orders_1h_without_refunds'] > tv) & (X['order_loan'] > 2000), action='block')
